...
#pragma acc parallel loop present(H[:ny+2][:nx+2], U[:ny+2][:nx+2], V[:ny+2][:nx+2])
      ...
      //first pass
      //x direction
#pragma acc parallel loop collapse(2) present(H[:ny+2][:nx+2], U[:ny+2][:nx+2], V[:ny+2][:nx+2], Hx[:ny][:nx+1], Ux[:ny][:nx+1], Vx[:ny][:nx+1])
      for (int j = 0; j < ny; j++) {
        for (int i = 0; i<=nx;i++) {
          //density calculation
      ...
      //y direction
#pragma acc parallel loop collapse(2) present(H[:ny+2][:nx+2], U[:ny+2][:nx+2], V[:ny+2][:nx+2], Hy[:ny+1][:nx], Uy[:ny+1][:nx], Vy[:ny+1][:nx])
      for(int j = 0; j<=ny; j++){
        for(int i=0; i<nx; i++){
          //density calculation
      ...
      //second pass
#pragma acc parallel loop collapse(2) present(H[:ny+2][:nx+2], U[:ny+2][:nx+2], V[:ny+2][:nx+2], Hnew[:ny+2][:nx+2], Unew[:ny+2][:nx+2], Vnew[:ny+2][:nx+2], Hx[:ny][:nx+1], Ux[:ny][:nx+1], Vx[:ny][:nx+1], Hy[:ny+1][:nx], Uy[:ny+1][:nx], Vy[:ny+1][:nx])
      for (int j = 1; j <=ny; j++) {
        for (int i = 1; i<=nx;i++) {
          //density calculation
